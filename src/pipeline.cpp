...
namespace scandium{
	...
	ScandiumPipeline::~ScandiumPipeline(){
	...
	void ScandiumPipeline::createGraphicsPipeline(const std::string& vertFilepath, 
	  const std::string& fragFilepath, const PipelineConfigInfo& configInfo){
		...
		if(vkCreateGraphicsPipelines(engineDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS){
			throw std::runtime_error("faile to create graphics pipeline");
		}

	}

	void ScandiumPipeline::createShaderModule(const std::vector<char>& code, VkShaderModule* shaderModule){
	...
	void ScandiumPipeline::bind(VkCommandBuffer commandBuffer){
		vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline);
	}

	PipelineConfigInfo ScandiumPipeline::defaultPipelineConfigInfo(uint32_t width, uint32_t height){
		...
		configInfo.viewport.x = 0.0f;
  		...
  		configInfo.viewport.width = static_cast<float>(width);					// This squishes
  		configInfo.viewport.height = static_cast<float>(height);
  		configInfo.viewport.minDepth = 0.0f;
  		configInfo.viewport.maxDepth = 1.0f;
 
  		configInfo.scissor.offset = {0, 0};										// This cuts
  		configInfo.scissor.extent = {width, height};

		configInfo.rasterizationInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;
		configInfo.rasterizationInfo.depthClampEnable = VK_FALSE;				// Enable for either 0 or 1 z values
		configInfo.rasterizationInfo.rasterizerDiscardEnable = VK_FALSE;		// Enable to only do the first stages of graphics process
		configInfo.rasterizationInfo.polygonMode = VK_POLYGON_MODE_FILL;		// Drawing lines vs corners vs filling
